...
ssh root@192.168.1.123 'rm -rf /var/lib/docker/containers/$(cat /opt/cuberite/devin-docker-id)/checkpoints && docker checkpoint create --leave-running cuberite checkpoint' && \
...
docker start --checkpoint=checkpoint cuberite && \
ssh root@192.168.1.123 'docker stop cuberite'
